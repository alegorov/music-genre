...
class TaggingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        l = embeds.shape[0]
        if l != 404:
            embeds = np.vstack([embeds, np.zeros((404 - l, 768), dtype=np.float32)])
        mask = np.ones(404, dtype=np.int32)
        mask[:l] = 0
        ...
        if self.testing:
            return track_idx, embeds, mask.tolist()
        tags = [int(x) for x in row.tags.split(',')]
        target = np.zeros(NUM_TAGS, dtype=np.float32)
        target[tags] = 1
        return track_idx, embeds, mask.tolist(), target
...
class BasicNet(nn.Module):
    def __init__(self):
        ...
        self.transformer = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(embedding_size, nhead=nhead, dim_feedforward=dim_feedforward, dropout=0.25,
...
def train_epoch(model, loader, criterion, optimizer):
    ...
    for iteration, data in enumerate(loader):
        optimizer.zero_grad()
        track_idxs, embeds, mask, target = data
        pred_logits = model(embeds, mask)
...
def collate_fn(b):
    ...
    mask = torch.tensor([x[2] for x in b], dtype=torch.bool, device=CUDA_DEV)

    if len(b[0]) == 3:
        return track_idxs, embeds, mask

    targets = torch.tensor(np.array([x[3] for x in b], dtype=np.float32), device=CUDA_DEV)
    return track_idxs, embeds, mask, targets
